...
CI SHALL promote frontend assets from the artifact bucket's `stable/` prefix to an environment's S3 static website bucket using `aws s3 sync`.

#### Scenario: Staging bucket is populated from stable prefix
- **WHEN** the CD pipeline runs the staging deploy step
- **THEN** `aws s3 sync s3://artifact-bucket/stable/ s3://staging-frontend-bucket/` SHALL execute and all files SHALL be present in the staging bucket

#### Scenario: Production bucket is populated from stable prefix after tests pass
- **WHEN** acceptance tests pass against staging
- **THEN** `aws s3 sync s3://artifact-bucket/stable/ s3://production-frontend-bucket/` SHALL execute
